...
function getTripData($trips, $startidx, $count, $type) {
  ...
  while ( $coord = $coords_result->fetch_object( 'Coord' ) ) {
    ...
    // new trip
    if ((($trip_id != $last_trip_id)&& ($firstLoop!=true))) {
      if ($type=="map") {
        $dataString .= "var myLine" . $last_trip_id . " = new google.maps.Polyline({
          clickable: true,
          map: map,
          path: myPoints" . $last_trip_id . ",
...
<?php if ($type=="map" && $submit) { ?>
  <script type="text/javascript" src="http://maps.google.com/maps/api/js?key=AIzaSyAvj5ZOIASf-08ujN3nH8hvuzduE_Nx8Mk"></script>
